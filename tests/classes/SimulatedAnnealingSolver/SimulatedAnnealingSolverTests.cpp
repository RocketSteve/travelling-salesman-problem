...
TEST_CASE("probabilityAccept", "[SimulatedAnnealingSolver]") {
    Instance *instance = new InstanceFileReader("resources/instance.txt");
    auto simulatedAnnealing = new SimulatedAnnealingSolver(instance, 1, 1, 1, 1);
    simulatedAnnealing->calculateDistance();
    auto distance = simulatedAnnealing->getDistance();
    REQUIRE(simulatedAnnealing->probabilityAccept(distance - 1) == exp(1));
    REQUIRE(simulatedAnnealing->probabilityAccept(distance) == 1);
}
